...
func (c *Config) ConvertFromPydioConfig(pydioConfig map[string]string ){
	...
	for i := 0; i < 10; i++ {
		...
		if _,ok := pydioConfig[MappingLocalType]; ok {
			c.Mappings[i].RightAttribute = pydioConfig[MappingLocalType]
		}
		if _,ok := pydioConfig[MappingLdapParam]; ok {
			c.Mappings[i].LeftAttribute = pydioConfig[MappingLdapParam]
		}
		if _,ok := pydioConfig[MappingLocalParam]; ok {
			c.Mappings[i].RuleString = pydioConfig[MappingLocalParam]
		}
	}
...
func convert(ldap string) string {
	switch ldap {
	case condition:
