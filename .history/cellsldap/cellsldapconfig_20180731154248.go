...
func (c *Config) ConvertFromPydioConfig(pydioConfig map[string]interface{}) {
	...
	for i := 0; mappingLoop; i++ {
		...
		if strings.ToLower(pydioConfig[MappingLdapParam].(string)) == "memberof" {
			fmt.Printf("%v", pydioConfig)
			fmt.Println("=====>LDAP_GROUP_FILTER" + pydioConfig["LDAP_GROUP_FILTER"].(string))
			// Mapping Group
test := pydioConfig["LDAP_GROUP_FILTER"]
			c.MemberOfMapping.GroupFilter.Filter = castString(pydioConfig["LDAP_GROUP_FILTER"])
