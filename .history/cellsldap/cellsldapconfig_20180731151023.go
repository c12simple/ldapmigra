...
func (c *Config) ConvertFromPydioConfig(pydioConfig map[string]interface{}) {
	...
	for i := 0; mappingLoop; i++ {
		if i == 0 {
			...
			index = "_strconv.Itoa(i)
		}
		MappingLocalParam := LOCAL_PARAM + "_" + index
		MappingLocalType := LOCAL_TYPE + "_" + index
		MappingLdapParam := LDAP_PARAM + "_" + index
		...
		if strings.ToLower(pydioConfig[MappingLdapParam].(string)) == "memberof" && strings.ToLower(pydioConfig[MappingLocalType].(string)) == "role_id" {
			// Mapping Group
